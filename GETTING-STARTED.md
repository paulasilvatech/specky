...
# Bootstrap each project — choose your target harness:
cd your-project
...
### What `specky install --target=copilot` creates
...
### What `specky install --target=claude` creates
...
2. Bootstrap the project: `specky install --target=copilot` (or `--target=claude`, `--target=cursor`, `--target=opencode`, `--target=agent-skills`)
