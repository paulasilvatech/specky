...
function resolveContext() {
    ...
    const requestedFeatureNumber = process.env.SDD_FEATURE_NUMBER
        ?? findString(input, ["feature_number", "featureNumber"]);
    ...
    const matches = readdirSync(specRoot, { withFileTypes: true })
        .filter((entry) => entry.isDirectory() && entry.name.startsWith(`${requestedFeatureNumber}-`));
    if (matches.length !== 1) {
        fail(`feature ${requestedFeatureNumber} must resolve to exactly one directory in ${specDir}; found ${matches.length}.`);
    }
    ...
    if (!existsSync(statePath) || !existsSync(signaturePath)) {
        fail(`feature ${requestedFeatureNumber} requires signed v5 state at ${featureDirectory}/.sdd-state.json.`);
    }
    ...
    if (!safeEqualHex(storedSignature, expectedSignature)) fail("feature state signature verification failed.");
    ...
    if (!Array.isArray(state.contract.phases) || !state.contract.phases.includes(state.current_phase)) {
        fail(`current phase ${String(state.current_phase)} is not enabled by the feature contract.`);
    }

    const capabilities = Array.isArray(state.contract.capabilities) ? state.contract.capabilities : [];
