...
describe("opencode target install", () => {
    ...
    it("installs full OpenCode primitive set with no cross-target leakage", async () => {
        ...
        expect(countFiles(resolve(workspace, ".opencode/agents"), (n) => n.startsWith("specky-") && n.endsWith(".md"))).toBe(13);
        expect(countFiles(resolve(workspace, ".opencode/commands"), (n) => n.startsWith("specky-") && n.endsWith(".md"))).toBe(22);
    ...
    it("emits native OpenCode tool tokens without cross-target leakage", async () => {
        ...
        const agent = readFileSync(resolve(workspace, ".opencode/agents/specky-orchestrator.md"), "utf8");
