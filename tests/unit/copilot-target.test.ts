...
function readCopilotManifest(): Record<string, HookGroup[]> {
  return JSON.parse(readFileSync(resolve(REPO, "dist/copilot-hooks.json"), "utf8")) as Record<
    string,
    HookGroup[]
  >;
}
...
describe("copilot target install", () => {
  ...
  it("installs full Copilot primitive set with no cross-target leakage", async () => {
    await expect(
      runInit({ target: "copilot", force: true, dryRun: false, workspace }),
    ).resolves.toBe(0);
    ...
    expect(
      countFiles(
        resolve(workspace, ".github/prompts"),
        (n) => n.startsWith("specky-") && n.endsWith(".md"),
      ),
    ).toBe(22);
    ...
    expect(
      countFiles(
        resolve(workspace, ".github/hooks/specky/scripts"),
        (n) => n.endsWith(".sh") || n.endsWith(".mjs"),
      ),
    ).toBe(17);
    expect(
      existsSync(resolve(workspace, ".github/hooks/specky/scripts/specky-contract-context.bash")),
    ).toBe(true);
    ...
    const instruction = readFileSync(
      resolve(instructionDir, "copilot-instructions.instructions.md"),
      "utf8",
    );
