...
import {
  type MigrateContractsOptions,
  runMigrateContracts,
} from "../../src/cli/commands/migrate-contracts.js";
...
describe("migrate-contracts", () => {
  ...
  function seedSingleRootState(state: Record<string, unknown> = legacyState()): string {
    ...
    writeFileSync(
      join(workspace, ".specs/.sdd-state.json"),
  ...
  it("requires per-feature mappings in a multi-feature workspace", async () => {
    ...
    await expect(runMigrateContracts(options())).rejects.toThrow(
      /Multiple features require --mapping/,
    );
  });
