...
describe("specky install target selection", () => {
  ...
  it("installs Cursor assets and MCP config with --target=cursor", async () => {
    ...
    expect(existsSync(resolve(workspace, ".agents/skills/specky-quality-reviewer/SKILL.md"))).toBe(
      true,
    );
  ...
  it("installs only shared skills with --target=agent-skills", async () => {
    await expect(
      runInit({ target: "agent-skills", force: true, dryRun: false, workspace }),
    ).resolves.toBe(0);

    expect(existsSync(resolve(workspace, ".agents/skills/specky-onboarding/SKILL.md"))).toBe(true);
    expect(existsSync(resolve(workspace, ".github/agents/specky-orchestrator.agent.md"))).toBe(
      false,
    );
  ...
  it("strips Claude hooks whenever Copilot is installed in the same workspace", async () => {
    await expect(runInit({ target: "both", force: true, dryRun: false, workspace })).resolves.toBe(
      0,
    );
