...
function readClaudeManifest(): Record<string, HookGroup[]> {
  return JSON.parse(readFileSync(resolve(REPO, "dist/claude-hooks.json"), "utf8")) as Record<
    string,
    HookGroup[]
  >;
}
...
describe("claude target install", () => {
  ...
  it("installs full Claude primitive set with a neutral rule and no Copilot leakage", async () => {
    await expect(
      runInit({ target: "claude", force: true, dryRun: false, workspace }),
    ).resolves.toBe(0);

    expect(
      countFiles(
        resolve(workspace, ".claude/agents"),
        (n) => n.startsWith("specky-") && n.endsWith(".md"),
      ),
    ).toBe(13);
    expect(
      countFiles(
        resolve(workspace, ".claude/commands"),
        (n) => n.startsWith("specky-") && n.endsWith(".md"),
      ),
    ).toBe(22);
    ...
    expect(
      countFiles(
        resolve(workspace, ".claude/hooks/scripts"),
        (n) => n.endsWith(".sh") || n.endsWith(".mjs"),
      ),
    ).toBe(17);
    expect(
      existsSync(resolve(workspace, ".claude/hooks/scripts/specky-contract-context.bash")),
    ).toBe(true);
