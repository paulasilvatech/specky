...
describe("lifecycle and workload pipeline profiles", () => {
    it("materializes a distinct evidence-oriented discovery pack for every workload", () => {
        ...
        for (const workload of workloads) {
            ...
            diagramManifests.add(resolved.required_diagrams.map((diagram) => `${diagram.type}:${diagram.source}`).join("|"));
        }
    ...
    it("requires a codebase baseline for brownfield discovery", () => {
        expect(() => discoveryQuestionsForContract(contract("brownfield", "service"), {
        ...
        expect(questions.some((question) => question.category === "Brownfield compatibility")).toBe(true);
    });

    it("requires explicit source and target evidence for migration discovery", () => {
        expect(() => discoveryQuestionsForContract(contract("migration", "api"), {
