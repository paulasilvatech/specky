...
function main() {
  ...
  if (apmVersion !== pkg.version) {
    errors.push(`apm.yml version "${apmVersion}" != package.json version "${pkg.version}"`);
  }
  if (configVersion !== pkg.version) {
    errors.push(`config.yml version "${configVersion}" != package.json version "${pkg.version}"`);
  }
  if (mcpPackage !== expectedRuntime) {
    errors.push(`mcp.json specky runtime "${mcpPackage ?? "missing"}" != "${expectedRuntime}"`);
  }
