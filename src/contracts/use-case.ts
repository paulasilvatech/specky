...
import { TRIVIAL_PROPERTY_BODY_PATTERN, TRIVIAL_TEST_BODY_PATTERN } from "../utils/test-quality.js";
  ...
  .object({
    ...
    figma: z
      .object({
        ...
        diagram_types: z
          .array(z.enum(["architecture", "user_flow", "data_flow", "integration"]))
          .min(1),
      })
      .strict()
      .optional(),
...
export type DocumentationConfig = NonNullable<
  NonNullable<CapabilityConfig["release"]>["documentation"]
>;

export const useCaseSelectionSchema = z
  .object({
...
export const resolvedUseCaseContractSchema = useCaseSelectionSchema
  .extend({
    id: z.string().min(1),
...
const LIFECYCLE_CONTRACTS: Record<Lifecycle, LifecycleContract> = {
  ...
  brownfield: {
    requiredDiscoveryArtifacts: ["RESEARCH.md", "CODEBASE_BASELINE.md", "COMPATIBILITY.md"],
  },
  migration: {
    requiredDiscoveryArtifacts: [
      "RESEARCH.md",
      "SOURCE_INVENTORY.md",
      "TARGET_INVENTORY.md",
      "MIGRATION_PLAN.md",
...
const SUPPORTED_CONTRACT_IDS = [
  "greenfield-api-full",
  "greenfield-api-rapid",
  "greenfield-api-emergency",
  "greenfield-web-application-full",
  "greenfield-web-application-rapid",
  "greenfield-web-application-emergency",
  "greenfield-service-full",
  "greenfield-service-rapid",
  "greenfield-service-emergency",
  "greenfield-cli-full",
  "greenfield-cli-rapid",
  "greenfield-cli-emergency",
  "greenfield-library-full",
  "greenfield-library-rapid",
  "greenfield-library-emergency",
  "greenfield-infrastructure-full",
  "greenfield-infrastructure-rapid",
  "greenfield-infrastructure-emergency",
  "brownfield-api-full",
  "brownfield-api-rapid",
  "brownfield-api-emergency",
  "brownfield-web-application-full",
  "brownfield-web-application-rapid",
  "brownfield-web-application-emergency",
  "brownfield-service-full",
  "brownfield-service-rapid",
  "brownfield-service-emergency",
  "brownfield-cli-full",
  "brownfield-cli-rapid",
  "brownfield-cli-emergency",
  "brownfield-library-full",
  "brownfield-library-rapid",
  "brownfield-library-emergency",
  "brownfield-infrastructure-full",
  "brownfield-infrastructure-rapid",
  "brownfield-infrastructure-emergency",
  "migration-api-full",
  "migration-api-rapid",
  "migration-api-emergency",
  "migration-web-application-full",
  "migration-web-application-rapid",
  "migration-web-application-emergency",
  "migration-service-full",
  "migration-service-rapid",
  "migration-service-emergency",
  "migration-cli-full",
  "migration-cli-rapid",
  "migration-cli-emergency",
  "migration-library-full",
  "migration-library-rapid",
  "migration-library-emergency",
  "migration-infrastructure-full",
  "migration-infrastructure-rapid",
  "migration-infrastructure-emergency",
] as const;
...
function validateCapabilityKeys(selection: UseCaseSelection): void {
  const capabilities = [...new Set(selection.capabilities)].sort((left, right) =>
    left.localeCompare(right),
  );
  const configured = Object.keys(selection.capability_config).sort((left, right) =>
    left.localeCompare(right),
  );
...
function validateTddConfig(selection: UseCaseSelection): void {
  ...
  for (const binding of tdd.property_bindings) {
    if (!binding.body.includes(binding.requirement_id)) {
      throw new Error(
        `Property binding ${binding.requirement_id} must carry its requirement ID in executable code.`,
      );
    }
    if (TRIVIAL_PROPERTY_BODY_PATTERN.test(binding.body)) {
      throw new Error(
        `Property binding ${binding.requirement_id} contains a placeholder or trivial property.`,
      );
    }
  }
}
...
export function resolveUseCaseContract(selection: UseCaseSelection): ResolvedUseCaseContract {
  ...
  const snapshot = {
    ...
    ...parsed,
    capabilities: [...new Set(parsed.capabilities)].sort((left, right) =>
      left.localeCompare(right),
    ) as Capability[],
