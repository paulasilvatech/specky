...
const documentationInputSchema = z
  .object({
...
function documentationConfig(toolName: string): {
  ...
  const context = requireFeatureContext(toolName);
  const release = requireCapabilityConfig(context.state.contract.capability_config, "release");
  return {
...
async function generateSingle(
  ...
  switch (type) {
    case "full":
      return docGenerator.generateFullDocs(
        context.featureDir,
        context.featureNumber,
        context.config,
      );
    case "api":
      return docGenerator.generateApiDocs(
        context.featureDir,
        context.featureNumber,
        context.config,
      );
    case "runbook":
      return docGenerator.generateRunbook(
        context.featureDir,
        context.featureNumber,
        context.config,
      );
    case "onboarding":
      return docGenerator.generateOnboarding(
        context.featureDir,
        context.featureNumber,
        context.config,
      );
...
async function writeSingle(
  ...
  return enrichResponse(
    toolName,
    {
      ...
      contract_id: requireFeatureContext(toolName).state.contract.id,
...
export function registerDocumentationTools(
  ...
  server.registerTool(
    TOOL_NAMES.GENERATE_DOCS,
    {
      ...
      description:
        "Assembles full feature documentation only when release.documentation enables full and all required specification, design, task, and analysis evidence exists.",
    ...
    async ({ force }) => {
      try {
        const result = await writeSingle(
          TOOL_NAMES.GENERATE_DOCS,
          "full",
          force,
          fileManager,
        ...
        return errorResult(TOOL_NAMES.GENERATE_DOCS, error);
      }
    },
  );
  ...
  server.registerTool(
    TOOL_NAMES.GENERATE_API_DOCS,
    {
      ...
      description:
        "Assembles API documentation from complete DESIGN.md endpoint contracts and the persisted API base URL; available only to API workloads with api documentation enabled.",
      inputSchema: documentationInputSchema,
      annotations: {
        readOnlyHint: false,
        destructiveHint: false,
    ...
    async ({ force }) => {
      try {
        ...
        return errorResult(TOOL_NAMES.GENERATE_API_DOCS, error);
      }
    },
  );
  ...
  server.registerTool(
    TOOL_NAMES.GENERATE_RUNBOOK,
    {
      ...
      description:
        "Assembles deployment, health, monitoring, troubleshooting, rollback, and support procedures from DESIGN.md evidence and persisted release documentation parameters.",
      inputSchema: documentationInputSchema,
      annotations: {
        readOnlyHint: false,
        destructiveHint: false,
    ...
    async ({ force }) => {
      try {
        ...
        return errorResult(TOOL_NAMES.GENERATE_RUNBOOK, error);
      }
    },
  );
  ...
  server.registerTool(
    TOOL_NAMES.GENERATE_ONBOARDING,
    {
      ...
      description:
        "Assembles onboarding from feature requirements, architecture, tasks, persisted setup steps, and support contacts; no package-manager or platform steps are inferred.",
      inputSchema: documentationInputSchema,
      annotations: {
        readOnlyHint: false,
        destructiveHint: false,
    ...
    async ({ force }) => {
      try {
        ...
        return errorResult(TOOL_NAMES.GENERATE_ONBOARDING, error);
      }
    },
  );
  ...
  server.registerTool(
    TOOL_NAMES.GENERATE_ALL_DOCS,
    {
      ...
      description:
        "Prevalidates every documentation type enabled by release.documentation, then writes the complete set atomically. Any generation or write failure leaves the prior docs unchanged.",
      inputSchema: documentationInputSchema,
      annotations: {
        readOnlyHint: false,
        destructiveHint: false,
      ...
      try {
        ...
        const result = await enrichResponse(
          TOOL_NAMES.GENERATE_ALL_DOCS,
          {
            ...
            contract_id: requireFeatureContext(TOOL_NAMES.GENERATE_ALL_DOCS).state.contract.id,
            documentation_version: context.config.version,
        ...
        return errorResult(TOOL_NAMES.GENERATE_ALL_DOCS, error);
      }
    },
  );
}
