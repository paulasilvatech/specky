  ...
  for (const file of policyAgentFiles(agentsDir)) {
    ...
    for (const server of declaredServers) {
      if (server !== "specky" && !allowedServers.has(server)) {
        errors.push(`agent "${file}" references optional MCP server "${server}" outside policy`);
      }
    }
  }
}
...
function checkHookEvents(policy: ApmPolicy, hooksManifest: string, errors: string[]): void {
  ...
  const manifestJson = JSON.parse(readFileSync(hooksManifest, "utf8")) as Record<string, unknown>;
  ...
  for (const file of agentFiles) {
    ...
    for (const token of forbidden) {
      if (line.includes(token)) {
        errors.push(`${targetName}: agent "${file}" compiles to a forbidden tool token "${token}"`);
      }
    }
  }
}
