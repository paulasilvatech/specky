...
type PlainLogicalTool = Exclude<LogicalTool, `mcp.specky.${string}` | `raw:${string}`>;
...
type PlainCapability = Exclude<AgentCapability, `mcp.specky.${string}` | `mcp.github.${string}`>;
...
export function isAgentCapability(value: string): value is AgentCapability {
  return value in CAPABILITY_NATIVE || /^mcp\.(specky|github)\.[a-z][a-z0-9_]*$/.test(value);
}
...
export function logicalToNative(logical: LogicalTool, target: HarnessTarget): string[] {
