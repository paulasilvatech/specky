...
export function agentCapabilities(content: string): AgentCapability[] {
  ...
  if (frontmatterValue(content, "tools") !== undefined) {
    throw new Error(
      "Canonical agent frontmatter must declare either capabilities or tools, not both.",
    );
  }
...
export function transformToolsLine(
  ...
  const mapped = unique(parseToolArray(rawTools).flatMap((token) => mapTool(token, target)));
  const tools =
    format === "comma-list" ? `tools: ${mapped.join(", ")}` : `tools: ${JSON.stringify(mapped)}`;
...
export function transformAgentTools(
  ...
  const mapped = unique(
    capabilities
      .filter(
        (capability) => !capability.startsWith("mcp.github.") || enabledIntegrations.has("github"),
      )
      .flatMap((capability) => capabilityToNative(capability, target)),
  );
  const tools =
    format === "comma-list" ? `tools: ${mapped.join(", ")}` : `tools: ${JSON.stringify(mapped)}`;
