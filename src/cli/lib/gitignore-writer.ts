...
const BLOCK_BODY = [
  ...
  "#   .cursor-plugin/plugin.json  Cursor plugin manifest (logo + MCP)",
...
export function writeGitignoreBlock(workspace: string, opts: { dryRun: boolean }): GitignoreResult {
  ...
  if (startIdx >= 0 && endIdx > startIdx) {
    ...
    if (!opts.dryRun) writeFileSync(path, updated, "utf8");
  ...
  if (!opts.dryRun) writeFileSync(path, appended, "utf8");
