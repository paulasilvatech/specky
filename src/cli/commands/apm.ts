...
import { generateLock, lockPath, serializeLock, verifyLock } from "../lib/apm/lock.js";
import { loadManifest, validateManifest } from "../lib/apm/manifest.js";
...
function runLock(pkgRoot: string, dryRun: boolean): number {
  ...
  console.log(
    `✅ Wrote apm.lock.yaml — ${count} primitives (${lock.package.name}@${lock.package.version})`,
  );
  return 0;
}
