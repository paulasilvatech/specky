...
import { type Phase, STATE_FILE } from "../../constants.js";
import {
...
function loadMappings(
  ...
  if (featureCount !== 1) {
    throw new Error(
      "Multiple features require --mapping=<json> with one explicit use-case selection per feature number.",
    );
  }
  if (!options.lifecycle || !options.workload || !options.executionMode) {
    throw new Error(
      "Single-feature migration requires --lifecycle, --workload, and --execution-mode.",
    );
  }
...
function featureDirectories(
  specRoot: string,
): Array<{ number: string; name: string; directory: string }> {
...
function copyLegacyPhases(state: SddState, source: Record<string, unknown>): void {
...
function toFeatureState(
  ...
  const state = stateMachine.createFeatureState({
    projectName:
      typeof source["project_name"] === "string" ? source["project_name"] : entry.feature_name,
    feature: {
  ...
  if (Array.isArray(source["amendments"]))
    state.amendments = source["amendments"] as SddState["amendments"];
  if (source["gate_decision"] !== undefined)
    state.gate_decision = source["gate_decision"] as SddState["gate_decision"];
  if (Array.isArray(source["gate_history"]))
    state.gate_history = source["gate_history"] as SddState["gate_history"];
  if (Array.isArray(source["drift_history"]))
    state.drift_history = source["drift_history"] as SddState["drift_history"];
  return state;
}
...
async function applyPlan(
  ...
  writeFileSync(
    join(backupRoot, "plan.json"),
    JSON.stringify({ ...plan, plan_hash: hash }, null, 2),
  );
  ...
  try {
    ...
    if (JSON.stringify(afterHashes) !== JSON.stringify(plan.artifact_hashes)) {
      throw new Error(
        "Artifact hash verification failed: non-state content changed during migration.",
      );
    }
