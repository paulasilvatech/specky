...
export const importDocumentInputSchema = z
  .object({
    file_path: z
      .string()
      .min(1)
  ...
  .strict()
  .describe("Import a document and convert it to Markdown for SDD processing.");

export const figmaToSpecInputSchema = z
  .object({
    figma_file_key: z.string().min(1).describe("Figma file key extracted from the Figma URL."),
    figma_node_id: z
      .string()
      .optional()
  ...
  .strict()
  .describe(
    "Extract design context from Figma and generate requirements specification. Outputs a payload for the AI client to call Figma MCP's get_design_context tool.",
  );

export const batchImportInputSchema = z
  .object({
    documents_dir: z
      .string()
      .min(1)
  ...
  .strict()
  .describe(
    "Batch import all documents from a directory, converting each to Markdown and processing through the SDD pipeline.",
  );
