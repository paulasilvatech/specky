...
export const importTranscriptInputSchema = z
  .object({
    file_path: z
      .string()
      .min(1)
...
export const autoPipelineInputSchema = z
  .object({
    file_path: z
      .string()
      .optional()
    ...
    format: z.enum(["vtt", "srt", "txt", "md", "auto"]).describe("Transcript format"),
    spec_dir: specDirSchema,
    force: z.boolean().describe("Explicit overwrite decision"),
  })
  .strict();

export const batchTranscriptsInputSchema = z
  .object({
    transcripts_dir: z
      .string()
      .min(1)
      .describe(
        "Path to folder containing transcript files (.vtt, .srt, .txt, .md) relative to workspace root. Example: 'transcripts' or 'OneDrive/Meeting Transcripts'",
      ),
    spec_dir: specDirSchema,
    use_case: useCaseSelectionSchema,
    features: z
