...
export function detectTechStackFromDesign(designContent: string): DesignTechStack | undefined {
  ...
  const namesJsRuntime =
    /\btypescript\b|\bjavascript\b|\bnode(\.js)?\b|\bdeno\b|\bbun\b|\bexpress\b|\bfastify\b|\bnext\.?js\b|\breact\b|\bnpm\b/.test(
      lower,
    );
  ...
  if (
    /\bgolang\b/.test(lower) ||
    (/\bgo\b/.test(lower) && !namesJsRuntime && !/\bjava\b/.test(lower))
  ) {
 ...
 */
export function detectServicesFromDesign(
  ...
  if (/\bpostgres(ql)?\b/.test(lower) || hasDep("pg") || hasDep("postgres") || hasDep("pg-promise"))
    services.push("postgres");
  if (/\b(mysql|mariadb)\b/.test(lower) || hasDep("mysql") || hasDep("mysql2") || hasDep("mariadb"))
    services.push("mysql");
  if (/\bmongo(db)?\b/.test(lower) || hasDep("mongodb") || hasDep("mongoose"))
...
> = {
  ...
  mongodb: {
    image: "mongo:7",
    ports: ["27017:27017"],
    volume: { name: "mongo-data", mountPath: "/data/db" },
  },
...
export class IacGenerator {
  ...
  async generateTerraform(cloud: CloudProvider, components: InfraComponent[]): Promise<IacResult> {
    ...
    return {
      ...
      next_steps:
        "Set required variables (see terraform.tfvars.example), then run sdd_validate_iac to validate the configuration via Terraform MCP.",
      diagram,
    };
  }
  ...
  generateDockerfile(
    ...
    if (includeCompose) {
      files.push({
        ...
        content: this.generateCompose(services),
    ...
    const serviceNote =
      includeCompose && services.length > 0 ? ` (services: app, ${services.join(", ")})` : "";
    ...
    return {
      ...
      next_steps:
        "Run docker build to test the image, or use sdd_setup_local_env to create a full dev environment.",
    };
  }
  ...
  generateDevcontainer(
    ...
    return {
      ...
      next_steps:
        "Use sdd_setup_codespaces to create a cloud environment, or open locally with 'Dev Containers: Reopen in Container'.",
    };
  }
  ...
  ): { mainTf: string; outputs: TfOutput[] } {
          ...
          outputs: [
            {
              ...
              value: "azurerm_servicebus_namespace.queue.id",
            },
          ],
        };
          ...
          outputs: [
            {
              ...
              value: "aws_dynamodb_table.nosql.name",
            },
          ],
        };
      case "database:sqlserver":
          ...
          outputs: [
            {
              ...
              value: "google_firestore_database.nosql.name",
            },
          ],
        };
      case "database:sqlserver":
          ...
          outputs: [
            {
              ...
              value: "google_logging_metric.logs.name",
            },
          ],
        };
      default:
  ...
  private generateVariables(cloud: CloudProvider, components: InfraComponent[]): IacVariable[] {
    ...
    const needsContainerVars = components.some(
      (c) =>
        c.module === "compute" &&
        (c.service === "container" || (c.service === "serverless" && cloud !== "azure")),
    );
    ...
    if (
      cloud === "aws" &&
      components.some((c) => c.module === "compute" && c.service === "kubernetes")
    ) {
      vars.push({
        name: "subnet_ids",
        type: "list(string)",
