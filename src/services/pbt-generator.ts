...
import { TRIVIAL_PROPERTY_BODY_PATTERN } from "../utils/test-quality.js";
...
export class PbtGenerator {
  ...
  private validateBindings(requirementIds: string[], bindings: PropertyBinding[]): void {
    ...
    if (missing.length > 0 || unknown.length > 0) {
      throw new Error(
        `Property bindings mismatch. Missing: ${missing.join(", ") || "none"}. Unknown: ${unknown.join(", ") || "none"}.`,
      );
    }
    ...
    for (const binding of bindings) {
      if (!binding.body.includes(binding.requirement_id)) {
        throw new Error(
          `Property binding ${binding.requirement_id} must include its requirement ID.`,
        );
      }
