...
import type { AlignmentCheck, CrossAnalysisResult } from "../types.js";
...
export class CrossAnalyzer {
  ...
  async analyze(featureDir: string): Promise<CrossAnalysisResult> {
    ...
    const designContent = await readSpecFileOrEmpty(this.fileManager, featureDir, "DESIGN.md");
    const tasksContent = await readSpecFileOrEmpty(this.fileManager, featureDir, "TASKS.md");
    ...
    const specDesign: AlignmentCheck[] = reqIds.map((id) => {
      ...
      return {
        source_id: id,
        target_id: found ? id : "",
        status: found ? ("aligned" as const) : ("missing" as const),
        detail: found ? `${id} referenced in DESIGN.md` : `${id} NOT found in DESIGN.md`,
      };
    });
    ...
    const designTasks: AlignmentCheck[] = reqIds.map((id) => {
      ...
      return {
        ...
        target_id: found ? id : "",
        status: found ? ("aligned" as const) : ("missing" as const),
        detail: found ? `${id} has implementing tasks` : `${id} has NO implementing tasks`,
      };
    });
    ...
    const orphanedTasks = this.extractTaskIds(tasksContent || "").filter((tid) => {
      ...
      return !reqIds.some(
        (rid) => (tasksContent || "").includes(`${tid}`) && (tasksContent || "").includes(rid),
      );
    });
  ...
  private generateTraceabilityDiagram(
    reqIds: string[],
    specDesign: AlignmentCheck[],
    designTasks: AlignmentCheck[],
  ): string {
