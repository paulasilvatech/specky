...
export class ModelRoutingEngine {
  ...
  private static readonly ROUTING_TABLE: Record<string, ModelRoutingHint> = {
    ...
    specify: {
      ...
      thinking: true,
      rationale:
        "Specify has high ambiguity and no executable feedback — top-tier reasoning justified.",
    ...
    clarify: {
      ...
      thinking: true,
      rationale:
        "Clarify resolves implicit constraints and contradictions — reasoning depth critical.",
    ...
    analyze: {
      ...
      thinking: true,
      rationale:
        "Analysis produces gate decisions (APPROVE/CONDITIONAL/REJECT) — deep reasoning mitigates false approvals.",
    ...
    implement: {
      ...
      thinking: false,
      rationale:
        "Implementation is iterative with test feedback — extended thinking actively harmful.",
      evidence_id: "arXiv:2502.08235",
      premium_multiplier: "1x",
    },
    verify: {
      ...
      thinking: true,
      rationale:
        "Verification maps every REQ-ID to test coverage — reasoning required to detect phantom completions.",
    ...
    release: {
      ...
      thinking: false,
      rationale:
        "Release artifacts (commit messages, PR descriptions, changelogs) need no reasoning depth.",
      evidence_id: "arXiv:2603.05344",
      premium_multiplier: "0.33x",
    },
  };
  ...
  getHint(phase: string, signals?: ComplexitySignal): ModelRoutingHint {
    const base =
      ModelRoutingEngine.ROUTING_TABLE[phase] ?? ModelRoutingEngine.ROUTING_TABLE["implement"]!;
  ...
  calculateCostSavings(teamSize: number, requestsPerDay = 10): CostAnalysis {
    ...
    const correctUnits = Math.round(teamSize * requestsPerDay * (0.6 * 1 + 0.3 * 3 + 0.1 * 0.33));
