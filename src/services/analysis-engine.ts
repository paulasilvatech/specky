...
export class AnalysisEngine {
  ...
  analyze(input: AnalysisInput): AnalysisResult {
    const { hasConstitution, hasSpec, hasDesign, hasTasks, specContent, designContent, tasksContent } = input;
    ...
    const traceMatrix = requirementRows.length > 0
      ? requirementRows
          .map((row) => `| ${row.reqId} | ${row.designMapped ? "Mapped" : "Missing"} | ${row.taskMapped ? "Mapped" : "Missing"} | Pending | ${row.status} |`)
          .join("\n")
