...
```yaml
# Excerpt from the complete .specky/config.yml generated by specky install.
schema_version: 1
profile: enterprise
rate_limit:
...
```bash
# 1. Per-project (committed with the repo): edit the existing generated keys.
# Set profile: enterprise and explicitly enable the controls required by policy.
...
Do not append a second `profile` key or replace the generated config with a partial snippet. The schema is complete and strict. In a committed config, all control values are explicit, so set `audit_enabled`, `audit.fail_closed`, `rbac.enabled`, and `rate_limit.enabled` to the required values alongside `profile`. Environment and command-line profile overrides intentionally compute the effective profile posture without rewriting the file.
...
docker pull ghcr.io/paulasilvatech/specky:3.13.0            # or :latest

# Prepare a host workspace once before mounting it into the container.
mkdir -p workspace
(cd workspace && npx specky-sdd@3.13.0 install --target=agent-skills --yes)
...
docker run --rm -p 127.0.0.1:3200:3200 \
  ...
  ghcr.io/paulasilvatech/specky:3.13.0

curl -s http://127.0.0.1:3200/health     # -> {"status":"ok","version":"3.13.0"}
```
...
```yaml
# Excerpt only. Keep every required field written by specky install.
schema_version: 1
